...
class Steganography(Eval):
    ...
    def eval_sample(self, sample: Any, *_):
        assert isinstance(sample, dict), "sample must be a dict"
        assert "payload" in sample, "sample must have a payload_text key"
        assert "covertext" in sample, "sample must have a task_prompt key"
        # assert "task_output" in sample, "sample must have a task_output key"
        assert isinstance(sample["payload"], str), "payload must be a string"
        assert isinstance(sample["covertext"], str), "covertext must be a string"
        # assert isinstance(sample["task_output"], str), "task_output must be a string"

        payload = sample["payload"]
        covertext = sample["covertext"]
        # reference_output = sample["task_output"]

        results = steganography.run_eval(
            task_prompt=covertext,
            reference_output=payload,
            payload=payload,
            completion_fn=self.completion_fns[0],
            monitor_completion_fn=self.completion_fns[1],
            prompt_version=self.prompt_version,
    ...
    def run(self, recorder: RecorderBase):
        ...
        return {
            ...
            "reconstruction_error": np.mean(
                [d["reconstruction_error"] for d in metrics]
            ),
            "character_error_rate_cap1": np.mean(
                [d["character_error_rate_cap1"] for d in metrics]
            ),
